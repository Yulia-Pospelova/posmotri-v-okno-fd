...
/* ЭЛЕМЕНТЫ СТРАНИЦЫ */
const cardsList = document.querySelector(".videos__list");
const cardsContainer = document.querySelector(".videos__list-container");
const videoContainer = document.querySelector(".search__media");
const videoElement = document.querySelector(".search__video");
...
async function mainMechanics(endpoint) {
  try {
    ...
    document
      .querySelectorAll(".video-card")[0]
      .classList.add("video-card_current");
    ...
    chooseCurrentVideo({
      ...
      cardLinksSelector: ".video-card",
      currentLinkClassName: "video-card_current",
...
function appendCards({ baseUrl, dataArray, cardTmp, container }) {
  dataArray.forEach((el) => {
    ...
    node.querySelector(".video-card__title").textContent = el.city;
    node.querySelector(".video-card__text").textContent = el.description;
    node
      .querySelector(".video-card__thumbnail")
